...
type Section = 'home' | 'team' | 'responsibilities' | 'miv' | 'downloads' | 'rich' | 'support' | 'social' | 'admin';
      ...
      <div className="p-4 border-t border-white/10 space-y-2">
        <button 
          ...
          className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold text-xs uppercase tracking-widest ${
            current === 'admin' 
              ? 'bg-meta-pink text-white shadow-lg' 
              : 'text-white/40 hover:text-meta-pink hover:bg-white/5'
          }`}
        >
          <Settings size={14} /> Área Restrita
        </button>
...
const AdminSection = () => (
  <div className="space-y-12 animate-in fade-in duration-500 max-w-4xl mx-auto">
    <header className="text-center space-y-4">
      <div className="w-20 h-20 bg-meta-pink/10 rounded-full flex items-center justify-center mx-auto text-meta-pink mb-4">
        <Lock size={40} />
      </div>
      <h2 className="text-4xl font-black text-meta-purple-deep uppercase tracking-tighter">Área Administrativa</h2>
      <p className="text-slate-500 text-lg font-medium">Acesso exclusivo para o time de Marketing gerenciar o portal.</p>
    </header>
    <div className="bg-white p-12 rounded-[48px] border border-slate-100 shadow-xl space-y-8">
      <div className="space-y-4">
        <label className="block text-sm font-black uppercase tracking-widest text-slate-400">Senha de Acesso</label>
        <input type="password" placeholder="Digite a chave mestre..." className="w-full px-8 py-5 rounded-2xl bg-slate-50 border-none text-lg focus:ring-4 focus:ring-meta-pink/20 transition-all font-bold" />
      </div>
      <button className="w-full bg-meta-purple-deep text-white py-5 rounded-2xl font-black text-xl hover:bg-meta-purple-base transition-all shadow-lg flex items-center justify-center gap-3">
        Acessar Painel de Controle <ArrowRight size={20} />
      </button>
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-slate-50 p-6 rounded-3xl text-center space-y-2">
          <FileSignature size={24} className="mx-auto text-meta-purple-base" />
          <h4 className="font-bold text-slate-900">Atualizar MIV</h4>
          <p className="text-xs text-slate-400">Upload de novas diretrizes</p>
        </div>
        <div className="bg-slate-50 p-6 rounded-3xl text-center space-y-2">
          <Users size={24} className="mx-auto text-meta-pink" />
          <h4 className="font-bold text-slate-900">Gerenciar Equipe</h4>
          <p className="text-xs text-slate-400">Fotos e bios do time</p>
        </div>
      </div>
    </div>
  </div>
